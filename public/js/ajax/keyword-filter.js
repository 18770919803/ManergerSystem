    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(datas.rows, function (index, value) {
                var time= exchangeTime(datas.rows[index].wordTime);
                var be = "<button class='btn btn-xs btn-primary editbtn'  onclick=\"edit('"+datas.rows[index].wordId+"','"+datas.rows[index].keyWord+"')\">编辑</button>";
                var de = "<button class='btn btn-xs btn-danger delebtn'  onclick='deleteone("+datas.rows[index].wordId+")';>删除</button>";
                var str =be+de;
                addBody(index, index+(pagesNum*20-19),"90");
                addBody(index, datas.rows[index].keyWord,trWidth);
    ...
    var pagefunction = function() {
        ...
        function reloadGrid() {
            $("#ID_gridTable").jqGrid({
                ...
                colNames: [  '关键词', '时间','操作'],
                colModel: [
                    { name: 'keyWord', index: 'keyWord', editable: true},
                    { name: 'wordTime', index: 'id', editable: true,formatter:function(cellvalue, options, rowObject){
                        return exchangeTime(cellvalue);
                    } },
                    { name: 'act', index: 'act', sortable: false,formatter:function(cellvalue, options, rowObject){

                        var wordId=rowObject.wordId;
