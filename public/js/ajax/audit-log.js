    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(datas.rows, function (index, value) {
                var time = exchangeTime(datas.rows[index].audit_time);
                addBody(index, index + (pagesNum * 20 - 19), "90");
                addBody(index, time, trWidth);
                addBody(index, datas.rows[index].user_name, trWidth);
                addBody(index, datas.rows[index].audit_ip, trWidth);
                addBody(index, datas.rows[index].audit_module, trWidth);
                addBody(index, datas.rows[index].audit_result, trWidth);
                addBody(index, datas.rows[index].audit_message, trWidth);
            });
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (index, obj) {
                ...
                if (moduleName == '全部') {
                    $("#module").append("<option value='' selected>" + moduleName + "</option>");
                } else {
        ...
        function reloadGrid() {
            $("#ID_gridTable").jqGrid({
                ...
                colModel: [
                    ...
                    {name: 'user_name', index: 'user_name', editable: true},
                    {name: 'audit_ip', index: 'audit_ip', editable: true},
                    {name: 'audit_module', index: 'audit_module', editable: true},
                    {name: 'audit_result', index: 'audit_result', editable: true},
                    {name: 'audit_message', index: 'audit_message', editable: true},
